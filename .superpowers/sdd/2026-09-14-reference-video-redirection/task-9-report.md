...
- Final race audit preserves exact provider result bytes in typed post-invoke state and rejects directory/artifact pathname replacement after pinned enumeration/hash.
- Final full archive: 512 tests passed; focused TaskService race suite: 8 tests passed. Worktree code is clean and no real provider call was made.
...
## Post-fix Download Race Hardening

- Replaced path-based download hierarchy checks with pinned descriptor-relative `O_DIRECTORY`/`O_NOFOLLOW` creation and validation for execution, download, shot, attempt, and target directories.
- Existing directories must be current-user owned with exact mode `0700`; broad, foreign, symlinked, or replaced entries fail closed without `chmod` mutation.
- `TaskService` pins both the target directory and its expected parent entry, validates that binding before and after enumeration, and reads artifacts through `O_NOFOLLOW` descriptors with pre/post `fstat`, final descriptor-relative identity/size/mode checks, exact-byte SHA-256, and no path resolution.
- Added deterministic directory replacement tests at download/shot/attempt/target levels, post-enumeration target replacement, post-hash artifact replacement, broad/foreign/symlink rejection, and exact success permissions.
