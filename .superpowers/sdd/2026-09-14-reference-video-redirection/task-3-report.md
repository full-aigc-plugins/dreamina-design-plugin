...
## Fix Round 5 — Immutable Published Orphans

### RED

```bash
python3 -m unittest tests.test_media_intake_service.MediaIntakeServiceTests.test_failure_after_publication_retains_original_private_digest_file tests.test_media_intake_service.MediaIntakeServiceTests.test_verification_failure_does_not_cleanup_replacement_path -v
```

Result: exit 1, `Ran 2 tests in 0.006s`, `FAILED (errors=1)`. The original published digest file was deleted after the injected validation failure; the pre-existing replacement-path/original-exception regression remained green.

### GREEN

```bash
python3 -m unittest tests.test_media_intake_service.MediaIntakeServiceTests.test_failure_after_publication_retains_original_private_digest_file tests.test_media_intake_service.MediaIntakeServiceTests.test_verification_failure_does_not_cleanup_replacement_path tests.test_media_intake_service.MediaIntakeServiceTests.test_concurrent_denial_cannot_remove_accepted_identical_source -v
```

Result: exit 0, `Ran 3 tests in 0.011s`, `OK`.
...
Result: exit 0, `Ran 47 tests in 0.150s`, `OK`.
...
Result: exit 0, `Ran 326 tests in 14.667s`, `OK`.

### Changes and self-review

- Removed `created`, `published_identity`, and the entire post-publication chmod/unlink cleanup branch.
- Once a content-addressed final pathname exists, every later verification, fsync, schema, or receipt-write failure leaves that pathname untouched. A newly published original remains `0400`; a replacement inserted before failure remains byte- and mode-identical.
- The original validation exception propagates without cleanup masking, and no receipt is persisted on failure.
- Attempt paths remain private and continue to be removed by the outer intake `finally` block.
- Existing identical-digest concurrent acceptance/denial behavior remains green.
- No Task 4 analysis files or behavior were modified.

### Concerns

- Failed post-publication attempts intentionally retain private content-addressed orphan files. Reclamation, if needed, requires a separately designed safe cleanup process.
