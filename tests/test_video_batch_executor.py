...
class Allowance:
    def __init__(self): self.reservations = []; self.ambiguous = []; self.commits = []
...
class Adapter:
    def __init__(self): self.calls = []; self.raise_after_invoke = None; self.status = "querying"; self.download = False
    def run(self, args):
        self.calls.append(list(args))
        if args[0] == "query_result":
            if self.download and "--download_dir" in args:
                target = Path(args[args.index("--download_dir") + 1]) / "clip.mp4"
                target.parent.mkdir(parents=True, exist_ok=True); target.write_bytes(b"\0\0\0\x18ftypisom" + b"x" * 20)
            return DreaminaResult(0, {"gen_status": self.status}, None, args[2], "")
        if self.raise_after_invoke: raise self.raise_after_invoke
        return DreaminaResult(0, {"submit_id": f"submit_{len(self.calls)}"}, None, f"submit_{len(self.calls)}", "")
...
class VideoBatchExecutorTests(unittest.TestCase):
    ...
    def test_success_download_is_verified_and_missing_artifact_is_explicit(self):
        ...
        result = self.executor.reconcile(PROJECT_ID, "v001")
        self.assertEqual(result["state"], "evaluation_retryable")
